...
export function SustainCard({ dps }: { dps: DpsResult }): ReactNode {
  ...
  // A pool whose ceiling is below one cast is the more serious finding of the two and has no
  // drain rate to rank by, so it wins the badge outright; otherwise the fastest drain does.
  const tooSmall = rows.find((row) => !row.holdsACast);
  ...
  return (
    <div className="card">
      <div className="row wrap gap-7 mb-4" style={{ alignItems: "baseline" }}>
        ...
        {(cost.manaSpentAs === "blood" || cost.energySpentAs === "blood") && (
          <span
            className="badge warn"
            title="`blood_user`: every mana and energy cost is paid from blood"
          >
            blood magic
          </span>
        )}
